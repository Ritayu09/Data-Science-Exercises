...
# Identifying the disease type makes this problem a multi-class problem
# 'Sick' variable is no more important in this part as we will convert the disease type into a numeric variable to act as class label
part_2 = cleaned_data.drop(['Sick'], axis = 1)
disease_type = {'Healthy': 0,
                 '21OHD-NC': 1,
                 '21OHD-C Disease': 2,
                 '3beta-HSD like': 3,
                 '3beta-HSD': 4,
                 ...
                 'POR': 8,
                 'NonCAH PAI': 9}
part_2['Disease'] = part_2['Disease'].map(disease_type)


# train test split with stratified sampling on 'Disease' variable

train, test = train_test_split(part_2, test_size = 0.20, stratify = part_2['Disease'], random_state = 10)

# normalizing data before training random forest classifier
...
# Final Model
best_classifier = RandomForestClassifier(n_estimators = 50, criterion = 'gini', max_depth=None, max_features= 'auto', min_samples_leaf= 1, min_samples_split= 2, random_state = 13)
